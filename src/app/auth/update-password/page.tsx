...
import { verifyResetToken, updatePasswordWithToken } from '@/lib/actions/auth.actions'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Card, CardContent } from '@/components/ui/card'
import { ArrowLeft, Lock, AlertCircle, Shield, Loader2 } from 'lucide-react'
...
function UpdatePasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updatePasswordWithToken(token, newPassword)
            ...
            <CardContent className="pt-6">
              {!tokenValid ? (
                ...
                <div className="space-y-6">
                  <div className="bg-[#FEE2E2] border-2 border-[#DC2626] rounded-lg p-6 text-center">
                    <AlertCircle className="w-16 h-16 text-[#DC2626] mx-auto mb-4" />
                    <h3 className="font-bold text-[#1F2A5A] text-lg mb-2">Invalid Reset Link</h3>
                  ...
                  <div className="space-y-3">
                    <Link href="/auth/reset-password">
                      <Button className="w-full h-12 bg-[#2563EB] hover:bg-[#1D4ED8]">
                        Request New Reset Link
                      </Button>
                    </Link>
                    <Link href="/auth/login">
                      <Button variant="outline" className="w-full h-12 border-[#E5E7EB]">
                        Back to Login
                      </Button>
                    </Link>
                  </div>
