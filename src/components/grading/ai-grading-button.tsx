...
export default function AIGradingButton({
  ...
  const handleGrade = async () => {
    ...
    try {
      const res = await onGrade(customRubric || undefined);
      setResult(res);

      if (res.success) {
        toast.success(`AI Grading Complete! Score: ${res.score}`);
        setTimeout(() => {
          setIsOpen(false);
          setCustomRubric("");
          setResult(null);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Use AI to automatically grade this submission. You can provide a custom grading
