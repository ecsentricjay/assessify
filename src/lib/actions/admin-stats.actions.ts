...
/**
 * Get CBT (Continuous Assessment Testing) related statistics
 */
export async function getCBTStats() {
  ...
  try {
    ...
    // Get CBT courses count
    const { count: cbtCourseCount } = await supabase
      .from('cbt_courses')
      .select('*', { count: 'exact', head: true })
      .eq('is_active', true)

    // Get total CBT revenue (from cbt_student_subscriptions)
    const { data: subscriptions } = await supabase
      .from('cbt_student_subscriptions')
      .select('amount_paid')
      .eq('is_active', true)

    const cbtRevenue = subscriptions?.reduce((sum, sub) => sum + (sub.amount_paid || 0), 0) || 0

    // Get unique students in CBT
    ...
    return {
      ...
      data: {
        ...
        cbtCourses: cbtCourseCount || 0,
        totalCBTRevenue: cbtRevenue,
        uniqueStudents: uniqueStudentIds.size,
      }
    }
  } catch (error: any) {
    console.error('[getCBTStats] Error:', error)
    return {
      success: false,
      error: error?.message || 'Failed to fetch CBT statistics'
    }
  }
}