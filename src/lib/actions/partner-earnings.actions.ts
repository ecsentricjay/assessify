 ...
 * Record partner earning when submission is made
 * Called from submission processing logic
 * ✅ NOW UPDATES REFERRALS TABLE STATS
 */
export async function recordPartnerEarning(data: {
  ...
  try {
    ...
    // Get partner info for this lecturer (WITH current stats)
    ...
    if (referralError || !referral || !partner || partner.status !== 'active') {
      ...
      console.log('❌ No active partner referral found for lecturer:', data.lecturerId)
    ...
    console.log('💰 Recording partner earning:', {
    ...
    // Record earning in partner_earnings table
    ...
    console.log('✅ Partner earning created:', earning.id)

    // ✅ UPDATE REFERRALS TABLE STATS (THIS IS THE FIX!)
    const now = new Date().toISOString()
    const isAssignment = data.sourceType === 'assignment_submission'
    
    const updatedStats = {
      total_submissions: (referral.total_submissions || 0) + 1,
      total_revenue: Number(referral.total_revenue || 0) + data.sourceAmount,
      partner_earnings: Number(referral.partner_earnings || 0) + partnerAmount,
      last_submission_at: now,
      first_submission_at: referral.first_submission_at || now,
      
      // Assignment-specific stats
      assignment_submissions: isAssignment 
        ? (referral.assignment_submissions || 0) + 1 
        : (referral.assignment_submissions || 0),
      assignment_revenue: isAssignment 
        ? Number(referral.assignment_revenue || 0) + data.sourceAmount 
        : Number(referral.assignment_revenue || 0),
      assignment_earnings: isAssignment 
        ? Number(referral.assignment_earnings || 0) + partnerAmount 
        : Number(referral.assignment_earnings || 0),
      
      // Test-specific stats
      test_submissions: !isAssignment 
        ? (referral.test_submissions || 0) + 1 
        : (referral.test_submissions || 0),
      test_revenue: !isAssignment 
        ? Number(referral.test_revenue || 0) + data.sourceAmount 
        : Number(referral.test_revenue || 0),
      test_earnings: !isAssignment 
        ? Number(referral.test_earnings || 0) + partnerAmount 
        : Number(referral.test_earnings || 0),
    ...
    if (updateError) {
      console.error('❌ Failed to update referral stats:', updateError)
      // Don't fail the entire operation, earning is already recorded
    } else {
      console.log('✅ Referral stats updated:', updatedStats)
    }
