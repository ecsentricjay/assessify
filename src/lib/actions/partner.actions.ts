 ...
 * ✅ FIXED: Now includes names and emails
 */
export async function getAllPartners() {
  try {
    ...
    if (partnersError) {
      console.error('[getAllPartners] Error:', partnersError)
    ...
    // Fetch all emails in batch
    const emailsMap = new Map<string, string>()
    try {
      for (const userId of userIds) {
        const { data: authUser } = await adminClient.auth.admin.getUserById(userId)
        if (authUser?.user?.email) {
          emailsMap.set(userId, authUser.user.email)
        }
      }
    } catch (emailError) {
      console.error('[getAllPartners] Email batch error:', emailError)
    }

    // Enrich partners with profile data
    const enrichedPartners = partners.map(partner => {
      const profile = partner.profile as any
      return {
        ...partner,
        first_name: profile?.first_name || '',
        last_name: profile?.last_name || '',
        full_name: profile ? `${profile.first_name || ''} ${profile.last_name || ''}`.trim() : '',
        email: emailsMap.get(partner.user_id) || '',
        phone: profile?.phone || partner.phone_number || ''
      }
    })
...
/**
 * Get partner by ID (admin view)
 * ✅ FIXED: Now includes name and email
 */
export async function getPartnerById(partnerId: string) {
  try {
    ...
    if (partnerError || !partner) {
      console.error('[getPartnerById] Error:', partnerError)
    ...
    let email = ''
    try {
      const { data: authUser } = await adminClient.auth.admin.getUserById(partner.user_id)
      email = authUser?.user?.email || ''
    } catch (emailError) {
      console.error('[getPartnerById] Email error:', emailError)
    }
...
/**
 * Get current logged-in partner's profile
 * ✅ FIXED: Now includes name and email from profiles table
 */
export async function getMyPartnerProfile() {
  try {
    ...
    // Get partner record with profile data
    const { data: partner, error: partnerError } = await supabase
      .from('partners')
      .select(`
        *,
    ...
    if (partnerError || !partner) {
      console.error('[getMyPartnerProfile] Error:', partnerError)
    ...
    try {
      ...
      console.error('[getMyPartnerProfile] Email error:', emailError)
    }
    ...
    console.error('[getMyPartnerProfile] Error:', error)
...
export async function registerPartner(data: {
  ...
  try {
    ...
    if (profileError) {
      console.error('Profile creation error:', profileError)
      return { error: 'Failed to create partner profile' }
    }
    ...
    if (partnerError || !partner) {
      console.error('Partner creation error:', partnerError)
      return { error: 'Failed to create partner account' }
    }
    ...
    return { error: 'Failed to register partner account' }
  }
}